...
export default function MergePdfPage() {
  ...
  const handleProcess = async () => {
    if (files.length < 2) {
      toast({ title: "Select at least 2 files", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      setResult({ url, filename: 'merged_document.pdf' });
    } catch (err) {
  ...
  if (result) {
    return (
      <ToolLayout title="Merge PDF" description="Combine PDFs in the order you want with the easiest PDF merger available.">
  ...
  return (
    <ToolLayout title="Merge PDF" description="Combine PDFs in the order you want with the easiest PDF merger available.">
      {files.length === 0 ? (
        <FileDropZone multiple accept="application/pdf" onFilesSelected={setFiles} />
      ) : (
        <div className="space-y-6 max-w-3xl mx-auto w-full">
          ...
          {isProcessing ? (
            <div className="p-8 bg-card border rounded-xl shadow-sm">
              <ProgressBar progress={45} label="Merging PDFs..." />
            </div>
          ) : (
